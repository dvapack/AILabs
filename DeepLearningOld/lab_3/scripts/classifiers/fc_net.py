...
def layernorm_forward_simple(x, gamma, beta, eps=1e-5):
...
def layernorm_backward_simple(dout, cache):
    ...
    xmu = x - mean
    dvar = np.sum(dxhat * xmu * -0.5 * inv_std**3, axis=1, keepdims=True)
    dmean = np.sum(-dxhat * inv_std, axis=1, keepdims=True) + dvar * np.mean(-2 * xmu, axis=1, keepdims=True)
    dx = dxhat * inv_std + dvar * 2 * xmu / D + dmean / D
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                gamma = self.params[f"gamma{i}"]
                beta = self.params[f"beta{i}"]
                out, norm_cache = layernorm_forward_simple(out, gamma, beta)
        ...
        dout, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = affine_backward(dout, final_cache)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.normalization == "batchnorm":
                ...
                dout, grads[f"gamma{i}"], grads[f"beta{i}"] = layernorm_backward_simple(dout, norm_cache)
