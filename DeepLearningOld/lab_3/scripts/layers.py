...
def batchnorm_backward(dout, cache):
    ...
    xmu = x - sample_mean
    dvar = np.sum(dxhat * xmu * -0.5 * inv_std**3, axis=0)
    dmean = np.sum(-dxhat * inv_std, axis=0) + dvar * np.mean(-2 * xmu, axis=0)
    dx = dxhat * inv_std + dvar * 2 * xmu / N + dmean / N
...
def spatial_groupnorm_backward(dout, cache):
    ...
    dmean = np.sum(-dx_hat_group * inv_std, axis=(2, 3, 4), keepdims=True) + dvar * np.sum(-2 * (x_group - mean), axis=(2, 3, 4), keepdims=True) / m
    dx_group = dx_hat_group * inv_std + dvar * 2 * (x_group - mean) / m + dmean / m
